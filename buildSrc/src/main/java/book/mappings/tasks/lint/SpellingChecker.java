...
import static book.mappings.tasks.lint.SpellingChecker.PluralSuffixes.IES;
import static book.mappings.tasks.lint.SpellingChecker.PluralSuffixes.SES;
import static book.mappings.tasks.lint.SpellingChecker.PluralSuffixes.VES;
...
public class SpellingChecker implements Checker<Entry<?>> {
    ...
    static {
        ...
        final InputStream parsedFile = SpellingChecker.class.getClassLoader().getResourceAsStream("minecraft_specific_words.txt");
        if (parsedFile == null) {
    ...
    private static void checkMapping(Entry<?> entry, String name, ErrorReporter errorReporter) {
        ...
        if (entry instanceof ClassEntry && name.contains(String.valueOf(PACKAGE_SEPARATOR))) {
            ...
            final String[] packageNames = name.substring(0, name.lastIndexOf(PACKAGE_SEPARATOR)).split(String.valueOf(PACKAGE_SEPARATOR));
    ...
    private static List<String> splitWords(List<String> words, Predicate<String> strippingCheck, String strippingRegex) {
        ...
        for (final String nameToSplit : words) {
            ...
            if (!nameToSplit.matches("[A-Z]+")) {
                ...
                final Set<String> split = Arrays.stream(nameToSplit.split("(?<=[a-zA-Z])(?=[A-Z])")).map(String::toLowerCase).collect(Collectors.toSet());
